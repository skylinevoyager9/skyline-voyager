...
export function FlightExtraBagsSection({ offer, selections, onChange }: Props) {
  ...
  return (
    <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6">
      <h2 className="font-display text-xl font-bold text-stone-900">Extra bags</h2>
      <p className="mt-2 text-sm text-stone-600">
        Optional paid baggage from the airline (when supported — e.g. British Airways in test).
        Included bags in your fare are shown above; extras booked here are added to your order.
      </p>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-stone-700">
                    ...
                    {svc.maximumQuantity > 1
                      ? ` · max ${svc.maximumQuantity}`
                      : ""}
                  </p>
                </div>
      ...
      {hasSelection ? (
        <p className="mt-4 text-sm text-stone-700">
          Bags subtotal (before agency fee):{" "}
      ...
      <p className="mt-3 text-xs text-stone-500">
        Not all airlines support extra bags via API yet. If none appear, this fare cannot add paid
        baggage at checkout.
      </p>
    </section>
  );
